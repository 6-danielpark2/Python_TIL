...
유니버설 함수: ndarray 안의 데이터 연소별로 연산을 수행하는 함수
...
배열 메서드이자 넘파이 네임스페이스 안에 있는 함수인 dot 함수를 이용해 행렬 곱셈을 계산해야 함

2차원 배열과 곱셈이 가능한 크기의 1 차워 배열 간의 행렬 곱셈의 결과는 1 차원 배열임
...
np.abs(walk) >= 10을 통해 처음 위치에서 10칸 이상 떨어진 시점을 알려주는 불리언 배열을 얻을 수 있음
